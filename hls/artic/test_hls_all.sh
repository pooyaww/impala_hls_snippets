...
do
    NAME=${TEST%.${EXTENSION}}
    ${FRONTEND} --hls-flags ${intfc} ${ANYDSL_INSTALL}/runtime/platforms/${FRONTEND}/*.impala ../${NAME}.${EXTENSION} --max-errors 1 --emit-llvm -o ${NAME} > hls_host_ir_${name}.dump
    RESULT=$?
...
do
    NAME=${TEST%.${EXTENSION}}
    ${FRONTEND} --hls-flags ${intfc} ../${STANDALONE}/${NAME}.${EXTENSION} --max-errors 1 --emit-llvm -o ${NAME} > hls_host_ir_${name}.dump
    RESULT=$?
    Name2Result+=([${NAME}]=${RESULT})
done
